...
export default function MeusPlanos() {
    ...
    return (
        <div className="flex w-full flex-col gap-4 rounded-2xl bg-white p-4 sm:h-[88vh]">
                    ...
                    <table className="w-full table-auto border-collapse text-left text-sm">
                        <thead>
                            <tr className="border-b bg-gray-100 text-gray-700">
                                ...
                                <th className="p-2 hidden sm:block">Valor</th>
                        ...
                        <tbody>
                            {apolices.map((apolice) => {
                                const ativo =
                                    new Date(apolice.data_inicio) <= new Date() &&
                                    new Date(apolice.data_fim) >= new Date();
                                return (
                                    <tr key={apolice.id} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="p-2">
                                            {apolice.plano?.seguradora?.nome ?? '---'}
                                        </td>
                                        <td className="p-2 hidden sm:block">
                                            <span className={`rounded-full px-2 py-1 text-xs font-semibold ${ativo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                {ativo ? 'Ativo' : 'Expirado'}
                                            </span>
                                        </td>
                                        <td className="p-2">
                                            {new Date(apolice.data_fim).toLocaleDateString()}
                                        </td>
                                        <td className="p-2 font-semibold text-[#0153A5] hidden sm:block">
                                            Kz{' '}
            ...
            {/* Resumo */}
            <div className="h-1/5 w-full rounded-2xl bg-gradient-to-bl from-[#1C8BF9] to-[#0153A5] p-4 text-white">
                <h1 className="mb-2 text-lg font-bold">Resumo Meus Planos</h1>
                <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
                    <div className="flex flex-col items-center">
                        <span className="text-sm">Planos Ativos</span>
                        <span className="text-2xl font-bold">{resumo.ativos}</span>
                    </div>

                    <div className="w-full sm:w-1/2">
                        <span className="mb-1 block text-center text-sm">Progresso</span>
                        ...
                        <span className="mt-1 block text-center text-xs">
                            {((resumo.ativos / resumo.total) * 100 || 0).toFixed(0)}%
                        </span>
                    </div>
