...
export default function PlaylistsList({ playlists, onSelecionar }: Props) {
  return (
    <div
      className="flex gap-4 overflow-x-auto pb-2 flex-nowrap"
      style={{
    ...
    >
      {playlists.map((playlist) => {
        const thumbnail = playlist.tumb ?? null; // já vem com asset() do backend
        ...
        return (
          ...
          >
            {thumbnail ? (
              <img
                src={thumbnail}
                alt={`Thumbnail da playlist ${playlist.nome}`}
                className="w-full h-110 object-cover bg-center"
              />
            ) : (
              <div className="w-full h-110 flex items-center justify-center bg-gray-300 text-gray-600">
                Sem capa
              </div>
            )}
