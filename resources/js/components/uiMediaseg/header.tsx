...
const menuItems = [
  ...
  { label: 'Aprender', options: ['Aprender'], rota: ['/cliente/aprender'] },
  { label: 'Blog', options: ['Blog'], rota: ['/cliente/blog'] },
];
...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 z-50 w-full h-20 bg-white shadow px-4 sm:px-16 flex items-center justify-between">
      <a href={route('home')}>
        <img src={logo} alt="Logo MediaSeg" className="h-8 sm:h-12" />
      </a>
          ...
          {menuItems.map((item) => (
            <li key={item.label} className="relative">
              ...
              >
                ...
                {item.options.length > 0 && <IoIosArrowDown />}
              </button>
